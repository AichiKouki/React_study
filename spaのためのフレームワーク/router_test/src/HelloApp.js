//URLに応じて複数のコンポーネントを切り替えて表示する。ここでは、日本語・英語・中国語で挨拶を表示するだけのコンポーネントを定義
...
//Routerタグは、React Royterライブラリからインポートしたもの。
//pathでは任意のパスを指定して、componentで作成したコンポーネントを指定。これで、パスごとに指定されたコンポーネントの内容が表示される
const HelloApp = () => (
  <Router>
    <div style={{margin: 20}}>
