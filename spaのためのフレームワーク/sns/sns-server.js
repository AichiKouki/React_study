...
// --------------------------------------------------------
// データベースに接続 --- (※1)
const db = require('./server/database') //データベースとのやりとりを行うモジュールを読み込む。(serverフォルダの「database.js」。)
...
// APIの定義
// ユーザ追加用のAPI - ユーザを新規追加する(友達追加ではなく、このサービスを使うユーザーを追加) --- (※3)
//useridを調べて、そのIDをすでに持ったユーザーがいないかを確認し、問題がなければ、ユーザーを追加するという処理
app.get('/api/adduser', (req, res) => {
  ...
  // 既存ユーザのチェック
  db.getUser(userid, (user) => {//db.getUserはdatabase.jsのメソッド
    if (user) { // 既にユーザがいる
      return res.json({status: false, msg: '既にユーザがいます'})
    }
    // 新規追加
    db.addUser(userid, passwd, (token) => {//db.addUserはdatabase.jsのメソッド
      if (!token) {
...
app.get('/api/login', (req, res) => {
  ...
  db.login(userid, passwd, (err, token) => {
    if (err) {
      res.json({status: false, msg: 'ユーザーIDまたはパスワードが違います。'})
      return
    }
    // ログイン成功したらトークンを返す
    res.json({status: true, token})
...
app.get('/api/add_friend', (req, res) => {
  ...
  db.checkToken(userid, token, (err, user) => {//ログインした後だから、認証トークンを調べる。ログイン後の処理は毎回これをする。
    if (err) { // 認証エラー
      res.json({status: false, msg: '認証エラー'})
      return
    }
    // 友達追加
    user.friends[friendid] = true //checkTokenにより得られた「user」のデータ使ってプロパティに代入
...
app.get('/api/add_timeline', (req, res) => {
  ...
  db.checkToken(userid, token, (err, user) => {//ここはログインした後の処理なので、また認証トークンを調べる
    if (err) {
      res.json({status: false, msg: '認証エラー'})
      return
    }
...
// ユーザの一覧を取得 --- (※7)。今回は、このサービスを使っている全てのユーザーが表示される
...
// 自分を含む友達のタイムラインを取得 --- (※9)。なお、時系列の新しい順に取得する
