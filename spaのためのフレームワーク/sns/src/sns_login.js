...
export default class SNSLogin extends Component {
  ...
  //「ユーザー登録」か「ログイン成功した時」にlocalStorageにIDとトークンを保存する。
  api (command) {//これはメソッド。渡される値「login」とか「adduser」とか。これによってアクセスするAPIを分ける。
    request
      .get('/api/' + command)
      .query({
...
//「localStorage」とは、Webブラウザーの管理するローカルPC内のストレージ領域に、データを保存できる機能。異なるサイトではそのデータは使えない。保存できる容量は5MBまで。
