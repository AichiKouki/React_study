...
export default class SNSTimeline extends Component {
  ...
  componentWillMount () {//コンポーネントが追加されたタイミング
    this.loadTimelines()
  }
  loadTimelines () { // タイムラインを取得 --- (※1)。ここが呼ばれるタイミングは、「componentWillMount()」と「書き込みが完了した」タイミング
    request
      .get('/api/get_friends_timeline') //サーバーAPIを用いて、タイムラインを取得。
      .query({
